...
const LoginSchema = z.object({
  ...
  photo_proof: z.string().max(2_000_000).optional(),
});
...
export async function POST(request: NextRequest) {
  ...
  const { emp_id, pin, photo_proof } = parsed.data;
  ...
  if (photo_proof && photo_proof.startsWith('data:image/')) {
    await query(
      `INSERT INTO login_photo_proofs (employee_id, image_data)
       VALUES (?, ?)`,
      [employee.id, photo_proof],
    );
  }
