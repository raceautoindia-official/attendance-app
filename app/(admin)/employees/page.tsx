...
export default function EmployeesPage() {
  ...
  const { data: shiftsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<ApiResponse<{ shifts: Array<{ id: number; name: string; type: string }> }>>;
    },
  });
  ...
  const { data: locationsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<ApiResponse<{ locations: Array<{ id: number; name: string }> }>>;
    },
  });
  const locations = locationsData?.data?.locations ?? [];

  // Add form
  const addForm = useForm<EmpForm>({
    ...
    <div className="space-y-4">
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add Employee" size="lg">
        <form onSubmit={addForm.handleSubmit((v: EmpForm) => addMutation.mutate(v))} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Assign Shift (optional)</label>
              <select {...addForm.register('shift_id')} className={selectClass}>
